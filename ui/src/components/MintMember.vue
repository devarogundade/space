            ...
            <div class="dao_form">
                <div class="form">
                    ...
                    <div class="navigation">
                        <RouterLink :to="`/app/daos/${$route.params.id}/members`" v-if="step == 1">
                            <button>
                                <IconArrowLeft :color="'#fff'" /> Cancel
                            </button>
                        </RouterLink>

                        <button v-on:click="mint()">Mint member
...
export default {
    ...
    methods: {
        ...
        mint: async function () {
            this.progress = true
            try {
                const trx = await Members.addMember(
                    this.tokenAllocations[0].address,
                    this.$toWei(this.tokenAllocations[0].tokens),
                    this.plugin,
                    this.dao[5]
                )
                ...
                if (trx && trx.transactionHash) {
                    messages.insertMessage({
                        ...
                        description: "You have successfully added a new member.",
                        type: "success",
                        linkTitle: "View Trx",
